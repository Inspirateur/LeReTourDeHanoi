from Model.class_AtkEffect import AtkEffect
...
class Atk:
	def __init__(
			self, nom, cd, width, height, images, dmg, knockback_x, knockback_y, weight, speed_x, speed_y, duration
	):
		self.nom = nom
		self.cd = cd
		self.timer = 0
	...
	def get_cd(self) -> float:
		return abs(self.timer)

	def get_maxCd(self) -> float:
		return abs(self.cd)
	...
	def cast(self, x, y, facing, combo, bonusSpeed_x, bonusSpeed_y):
		return AtkEffect(
			self.nom, x, y, self.width, self.height, self.images, self.dmg*combo,
			self.knockback_x*facing, self.knockback_y, self.weight, self.speed_x*facing+bonusSpeed_x,
			(self.speed_y+bonusSpeed_y), facing, self.duration
		)
